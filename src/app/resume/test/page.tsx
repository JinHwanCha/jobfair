...
export default function TestResumeApplyPage() {
  const { t } = useI18n();
      ...
      <main className="content-container">
        ...
        <div className="max-w-lg mx-auto mb-4">
          <div className="bg-red-50 border border-red-300 rounded-xl px-4 py-3 text-sm text-red-800 font-bold text-center">
            🧪 TEST MODE — 카운트다운 없이 자소서 신청 폼 미리보기
          </div>
        </div>
        ...
        {mentors.length > 0 && (
          <div className="mb-8">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {mentors.map(m => (
                <MentorCard key={m.id} mentor={m} onClickDetail={() => setSelectedMentor(m)} />
              ))}
            </div>
          </div>
        )}
          ...
          {step === 3 && (
            <div className="card max-w-lg mx-auto">
              ...
              <div className="mt-6">
                ...
                <textarea name="reviewGoal" value={formData.reviewGoal}
                  onChange={handleInputChange} placeholder={t('resume.reviewGoalPlaceholder')}
