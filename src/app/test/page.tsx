...
export default function TestApplyPage() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'birthDate') setBirthYearConfirmed(false);
      ...
      <div className="page-container">
        ...
        <main className="content-container">
          <div className="text-center py-16">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-4">{t('apply.successTitle')}</h1>
            <p className="text-gray-600 mb-8">{t('apply.successDesc')}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={() => router.push('/my')} className="btn-primary">{t('apply.checkResult')}</button>
              <button onClick={() => router.push('/')} className="btn-secondary">{t('apply.goHome')}</button>
            </div>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="page-container">
      ...
      <main className="content-container">
          ...
          {[1, 2, 3, 4, 5].map((num) => (
            <div key={num} className="flex items-center">
              <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm ${
                step >= num ? 'bg-primary-400 text-gray-900' : 'bg-warm-200 text-gray-500'
              }`}>{num}</div>
              {num < 5 && (
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-gray-200 pt-4 mt-2">
                ...
                <label className="flex items-center gap-3 cursor-pointer mb-3">
                  <input type="checkbox" name="isForeigner" checked={formData.isForeigner}
                    onChange={(e) => {
                      const checked = e.target.checked;
                      setFormData({ ...formData, isForeigner: checked, languageGroup: checked ? formData.languageGroup : '' });
                    }}
                ...
                {formData.isForeigner && (
                  <div className="ml-8 space-y-2">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name="languageGroup" value="english" checked={formData.languageGroup === 'english'}
                        onChange={() => setFormData({ ...formData, languageGroup: 'english' })}
                        className="w-4 h-4 text-primary-600 focus:ring-primary-500" />
                      <span className="text-sm text-gray-700">{t('apply.english')}</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name="languageGroup" value="chinese" checked={formData.languageGroup === 'chinese'}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="bg-warm-100 rounded-xl p-3 mt-1">
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => { setBirthYearConfirmed(true); setFormData({ ...formData, birthYear: derivedBirthYear }); }}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                        birthYearConfirmed && formData.birthYear === derivedBirthYear ? 'bg-primary-400 text-gray-900' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
                      }`}>{t('apply.birthYearYes')}</button>
                    <button type="button" onClick={() => { setBirthYearConfirmed(true); setFormData({ ...formData, birthYear: '' }); }}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                        birthYearConfirmed && formData.birthYear !== derivedBirthYear ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
                      }`}>{t('apply.birthYearNo')}</button>
                  </div>
                  {birthYearConfirmed && formData.birthYear !== derivedBirthYear && (
                    <div className="mt-3">
                      <input type="text" name="birthYear" value={formData.birthYear} onChange={handleInputChange}
                        placeholder={t('apply.birthYearManualPlaceholder')} maxLength={4} className="input-field" />
                    </div>
                  )}
                </div>
              </div>
              <div>
                ...
                <select name="currentStatus" value={formData.currentStatus}
                  onChange={(e) => setFormData({ ...formData, currentStatus: e.target.value })} className="input-field">
                ...
                <div className="space-y-2">
                  {(['career', 'employment', 'interview'] as MentoringTopic[]).map((topic) => (
                    <label key={topic} className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={formData.interestTopics.includes(topic)}
                        onChange={(e) => {
                          const checked = e.target.checked;
                          setFormData({
                            ...formData,
        ...
        {step === 3 && (
          <div>
            <div className="bg-primary-100 rounded-2xl p-4 mb-6">
              <p className="text-gray-800 text-sm font-medium mb-2">{t('apply.selectGuide')}</p>
              <p className="text-gray-700 text-xs">{t('apply.selectGuide2')}</p>
            </div>
              ...
              {Array.from({ length: 6 }, (_, i) => i + 1).map((num) => {
                ...
                return (
                  <div key={num} className={`p-2 sm:p-3 rounded-xl text-center relative ${mentor ? 'bg-primary-100' : 'bg-warm-100'}`}>
                    {mentor && (
                      <button onClick={() => handleSelectMentor(mentorId, num)}
                        ...
                        aria-label={`${num}지망 선택 취소`}>✕</button>
                    )}
            ...
            {isLoadingMentors ? (
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                {mentors.map((mentor) => {
                  ...
                  for (let i = 1; i <= 6; i++) {
                    if (formData[`choice${i}` as ChoiceKey] === mentor.id) { selectedAs = i; break; }
                  }
                  return (
                    <div key={mentor.id} className="relative min-w-0">
                      {selectedAs && (
                        <div className={`absolute -top-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold z-10 ${CHOICE_COLORS[selectedAs - 1].bg}`}>{selectedAs}</div>
                      )}
                      <MentorCard mentor={mentor} selected={isSelected} showSelectButton
                        onClickDetail={() => setDetailMentor(mentor)}
                        onSelect={() => {
                          for (let i = 1; i <= 6; i++) {
                            if (formData[`choice${i}` as ChoiceKey] === mentor.id) { handleSelectMentor(mentor.id, i); return; }
                          }
                          for (let i = 1; i <= 6; i++) {
                            if (!formData[`choice${i}` as ChoiceKey]) { handleSelectMentor(mentor.id, i); return; }
                          }
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-gray-200 p-4 z-30">
              <div className="max-w-2xl mx-auto flex gap-4">
                <button onClick={prevStep} className="btn-secondary flex-1">{t('apply.prev')}</button>
                <button onClick={nextStep} disabled={!allChoicesFilled}
                  className="btn-primary flex-1 disabled:opacity-50 disabled:cursor-not-allowed">{t('apply.next')}</button>
              </div>
            </div>
          </div>
        )}
                ...
                {Array.from({ length: 6 }, (_, i) => i + 1).map((num) => {
                  ...
                  return (
                    <div key={num} className={`${color.light} rounded-xl p-4`}>
                      ...
                      <textarea value={formData[msgKey]}
                        onChange={(e) => setFormData({ ...formData, [msgKey]: e.target.value })}
                        placeholder={`${mentor.name}${t('apply.messagePlaceholder')}`}
                        className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" rows={2} />
                    </div>
                  );
                })}
              </div>
        ...
        {step === 5 && (
          <div className="card max-w-md mx-auto">
              ...
              <div className="space-y-2 text-sm">
                ...
                {formData.isForeigner && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formData.languageGroup === 'english' ? t('apply.english') : t('apply.chinese')}</span>
                  </div>
                )}
                <div className="border-t border-gray-200 my-2 pt-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formData.interestTopics.map(topic => t(`apply.topic_${topic}` as Parameters<typeof t>[0])).join(', ')}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                return (
                  <div key={num} className={`p-3 rounded-xl ${color.light}`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold ${color.bg}`}>{num}</div>
                    ...
                    {message && <p className="text-xs text-gray-500 mt-2 ml-11 italic">&ldquo;{message}&rdquo;</p>}
                  </div>
                );
              })}
            </div>
            ...
            {submitError && <div className="bg-red-50 text-red-600 p-4 rounded-xl mb-6 text-sm">{submitError}</div>}
      ...
      {detailMentor && <MentorModal mentor={detailMentor} onClose={() => setDetailMentor(null)} />}
    </div>
  );
}
